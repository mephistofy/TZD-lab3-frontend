...
const ChatHeader = ({ chat }) => {
    ...
    const keyForChat = (id) => {
        const key = localStorage.getItem('id-' + chat.id);
        return key;
    }
    ...
    const setKeyToReadIncoming = () => {
        if (key.length < 1) return

        localStorage.setItem('gotKey-' + chat.id, key)
    }

    const handleKeyDown = (e) => {
        if (e.key === 'Enter') setKeyToReadIncoming()
    }
            ...
            <div id='chatter'>
                {
                    chat.Users.map(user => {
                        return <div className='chatter-info' key={user.id}>
                            ...
                            <h3> Your key for this chat is: {keyForChat(user.id)}</h3>
                           ...
                           <div id='key-input'>
                              <input
                                value={key}
                                type='text'
                                placeholder='Enter key from your friend'
